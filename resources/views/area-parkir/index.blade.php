    ...
    <div class="card">
        <div class="card-body">
            ...
            <div style="font-size:32px;font-weight:700;color:var(--text);margin-bottom:4px">
                {{ $area->terisi }}<span style="font-size:16px;font-weight:400;color:var(--text-muted)"> / {{ $area->kapasitas }}</span>
            </div>
            ...
            <div style="display:flex;justify-content:space-between;align-items:center">
                ...
                <div class="btn-group">
                    <a href="{{ route('area-parkir.edit', $area->id_area) }}" class="btn btn-warning btn-sm"><i class="fas fa-edit"></i></a>
                    @if($area->terisi == 0)
                    <form method="POST" action="{{ route('area-parkir.destroy', $area->id_area) }}" style="display:inline" onsubmit="return confirm('Hapus area ini?')">
                        @csrf @method('DELETE')
            ...
            @if($persen >= 90)
            <div style="margin-top:8px;padding:8px;background:#fef2f2;border-radius:6px;font-size:12px;color:var(--danger);font-weight:600">
