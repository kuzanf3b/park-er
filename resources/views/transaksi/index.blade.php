...
<div class="card mb-6" style="margin-bottom:24px">
    <div class="card-body" style="padding:16px 24px">
        <form method="GET" action="{{ route('transaksi.index') }}" style="display:flex;gap:12px;align-items:end;flex-wrap:wrap">
            <div style="flex:1;min-width:200px">
                <label class="form-label">Cari Plat Nomor</label>
                <input type="text" name="search" class="form-control" placeholder="Cari plat nomor..." value="{{ request('search') }}">
            </div>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($t->status === 'masuk')
                            <a href="{{ route('transaksi.keluar', $t->id_parkir) }}" class="btn btn-warning btn-sm">
                                <i class="fas fa-sign-out-alt"></i> Keluar
                            </a>
                        @else
                            <a href="{{ route('transaksi.receipt', $t->id_parkir) }}" class="btn btn-outline btn-sm">
